...
from chocomufin.actions import parse_table, get_hex
...
def control(ctx: click.Context, table: str, files: Iterable[str], ignore: str, parser: str = "alto"):
    control_table = parse_table(table, normalization_method=ctx.obj["unorm"])
    errors = defaultdict(list)
    missing_chars = set()
    for file in tqdm.tqdm(files):
        if ignore in file:  # Skip converted content
            continue
        new_chars = check_file(file, table=control_table, normalization_method=ctx.obj["unorm"], parser=Alto)
        if new_chars:
...
def convert(ctx: click.Context, table: str, files: Iterable[str], suffix: str = ".mufichecker.xml"):
    ...
    in file with SUFFIX."""
    control_table = parse_table(table, as_dict=True, normalization_method=ctx.obj["unorm"])

    for file in tqdm.tqdm(files):
        if suffix in file:  # Skip converted content
            continue

        instance: Parser = convert_file(file, control_table, normalization_method=ctx.obj["unorm"])
